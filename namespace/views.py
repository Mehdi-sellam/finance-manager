...
# Consolidated views matching urls.py expected names
class NamespaceListCreateView(APIView):
    ...
    def get(self, request):
        namespaces = list_namespaces(request.user)
        return Response(NamespaceSerializer(namespaces, many=True).data, status=status.HTTP_200_OK)

    @swagger_auto_schema(
        operation_summary="Create namespace",
        request_body=NamespaceCreateSerializer,
...
class NamespaceDetailView(APIView):
    ...
    @swagger_auto_schema(
        operation_summary="Retrieve namespace by ID",
    ...
    def get(self, request, pk):
        namespace = get_namespace(request.user, pk)
        return Response(NamespaceSerializer(namespace).data, status=status.HTTP_200_OK)

    @swagger_auto_schema(
        operation_summary="Update namespace by ID",
        request_body=NamespaceUpdateSerializer,
        responses={200: NamespaceSerializer},
        tags=["Namespaces"],
        security=[{"Token": []}],
    ...
    def patch(self, request, pk):
        serializer = NamespaceUpdateSerializer(data=request.data)
        if serializer.is_valid():
            current = get_namespace(request.user, pk)
            updated = update_namespace(request.user, current.name, new_name=serializer.validated_data["new_name"])
            return Response(NamespaceSerializer(updated).data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    @swagger_auto_schema(
        operation_summary="Delete namespace by ID",
    ...
    def delete(self, request, pk):
        current = get_namespace(request.user, pk)
        delete_namespace(request.user, current.name)
